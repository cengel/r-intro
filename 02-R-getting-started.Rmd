...
be read as `3` **goes into** `x`.  You can also use `=`
...
> <h3>Challenge</h3>
>
> Create two variables `r_length` and `r_width` and assign them values. It should be noted that, because length is a built-in R function, R Studio might add parenthesis "()" after you type length. If you leave the parentheses you will get unexpected results. This is why you might see abbreviations of common words in R code, for example `r_len`. Calculate the area based on the current values of `r_length` and `r_width` and assign the result to a new, third variable `r_area`. Demonstrate that changing the values of either `r_length` and `r_width` does not affect the value of `r_area`. Make ample use of comments in your code.
...
An example would be the function `sqrt()`, which calculates the square root. The
...
Here, we provide the number `64` as input to the `sqrt()` function, which
calculates the square root of the input value, and returns the result. We can also assign the value `64` to a variable  `a`, which is then given to the `sqrt()` function,we can also assign the output of the function to a variable, in this case a new variable `b`. 
...
`area_hectares`. Then we add the value `30` to the beginning, again saving the result back into `area_hectares`.
...
"or" operator `|` to test for equality to multiple values like so:
...
area_hectares[area_hectares >= 30 & area_hectares <= 90]
```
...
>
> 3. How may of the populations are of size 20 and over?
...
##    population <- c(10,24,NA,18,NA,20)
##
## 2. Use the function `median()` to calculate the median of the `population` vector.
```
