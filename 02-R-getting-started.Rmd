...
ones are matrices (`matrix`), s (`data.frame`), lists (`list`), and
factors (`factor`).
...
We will go into more detail about data frames and factors later on. For now, try the following:
...
```{r eval=FALSE}
install.packages("name_of_package_to_install", dependencies = TRUE)
```
...
```{r eval=FALSE}
library(name_of_package_to_install)
```
or
```{r eval=FALSE}
require(name_of_package_to_install)
```
...
> ## Challenge 3
>
> 1. Google for an R package that might be of interest for your work.
...
One of the most immediate ways to get help, is to use the RStudio help interface. This panel by default can be found at the lower right hand panel of RStudio. As seen in the screenshot, by typing the word "Mean", RStudio tries to also give a number of suggestions that you might be interested in. The description is then shown in the display window.
