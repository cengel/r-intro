...
You may know about [the Stanford Open Policing Project](https://openpolicing.stanford.edu) and we will be working a sample dataset from their repository (https://openpolicing.stanford.edu/data/). It contains information about traffic stops for blacks and whites in the state of Mississippi during January 2013 to mid-July of 2016. 
...
Download the traffic stop data from here: https://github.com/cengel/R-intro/raw/master/data/MS_trafficstops_bw.csv
...
Our gapminder data frame has rows and columns (it has 2 dimensions), if we want to
...
head_trafficstops <- trafficstops[1:6, ] # equivalent to head(gapminder)
```
...
```{r, eval=FALSE, purl=FALSE}
trafficstops[,-1]          # The whole data frame, except the first column
...
(like the country names in our example dataset).
...
analyses. If you have control over your data the best practice for dealing
...
```{r calculate-age, results='show', echo=FALSE, purl=TRUE}
## Challenge
##
